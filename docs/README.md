...
### 1.2 전체 로직
이 코드는 의약품 관련 텍스트 데이터를 처리하고 분석하는 것을 목적으로 합니다. 특히, OCR 기술을 사용하여 이미지에서 텍스트를 추출하고, 추출된 텍스트를 정제 및 분석하여 데이터베이스에 있는 의약품 이름과 매칭시키는 작업을 수행합니다.
처음에는 텍스트에서 의약품의 단위 정보를 추출하고 이를 표준화하는 과정을 거칩니다. 이후, 데이터베이스로부터 읽어온 의약품 정보와 함께 추출된 텍스트를 사용하여, 주어진 쿼리와 가장 유사한 의약품을 찾아내는 매칭 프로세스를 진행합니다. 한글 문자의 특성을 고려하여 초성, 중성, 종성으로 분리하고, 이를 기반으로 유사도를 측정합니다.
이 과정은 높은 정확도로 의약품을 식별하고, 의약품 데이터베이스를 최신 상태로 유지하기 위한 것입니다. 결과적으로 이 코드를 통해, 의약품 정보의 관리 및 검색 효율성을 향상되고 사용자가 의약품 관련 정보를 보다 쉽고 빠르게 접근할 수 있게 될 것이라고 예상합니다.
### 1.3 주요 프로세스
...
```
RxBagExtractor는 팀에서 자체 제작한 전처리용 파이썬 패키지입니다. 자세한 내용은 <a href="https://pypi.org/project/RxBagExtractor/">PyPI</a>에서 확인할 수 있습니다.
...
ITEM_NAME 컬럼 전처리에는 `preprocess_text` 함수를, ITEM_NAME_CHO, ITEM_NAME_JUNG, ITEM_NAME_JONG 컬럼을 위해서는 `extract_chojungsung` 함수를 정의하였습니다. 또한 Brand와 Unit 컬럼을 위해서 `extract_unit` 함수를 정의하였습니다. 본 문서에서는 약품명 DB 전처리를 위한 함수 로직에 대한 더 자세한 설명은 제공하지 않을 것입니다.
...
- <b> Example </b></i>
```python
# CHOSEONG_GROUPS contains ["ㄱ", "ㄲ", "ㅋ"] and  ["ㄴ"].
...
> phoneme_distance 함수를 기반으로 두 단어간 거리를 계산하는 함수
...
### 4.2 levenshtein(s1, s2)
> 
역할: 두 문자열 간의 레벤슈타인 거리(편집 거리)를 계산하여, 얼마나 유사한지 점수를 계산합니다. 이 함수는 주로 문자열 매칭 과정에서 사용됩니다.
사용 예: levenshtein('가나다', '가내다') → 1

find_best_match(line, medication_db, query_unit)

역할: 주어진 텍스트와 데이터베이스에서 가장 유사한 약물명을 찾습니다. 유사도가 높은 항목을 반환하며, 단위까지 일치하는지 확인합니다.
사용 예: find_best_match('20밀리그램 정', medication_db, '20밀리그램') → ('타이레놀', 4)
process_line(query)

역할: 입력된 텍스트를 전처리하고, 가장 적합한 약물명을 찾아 반환합니다. 텍스트 전처리, 단위 추출, 초성/중성 분석을 통해 매칭 과정을 수행합니다.
사용 예: process_line('타이레놀 20mg') → ('타이레놀', 4)
process_text(text)

역할: 다수의 텍스트 라인을 처리하여 각 라인별로 매칭된 약물명을 반환합니다. 전체 프로세스를 묶어 한 번에 실행하는 함수입니다.
사용 예: process_text('타이레놀 20mg\n아스피린 500mg') → ['타이레놀', '아스피린']
split_string_by_comma(string)

역할: 문자열을 쉼표(,)로 분할하여 리스트로 반환합니다.
사용 예: split_string_by_comma('타이레놀, 아스피린') → ['타이레놀', '아스피린']
calculate_metrics(row)

역할: 예측된 약물명과 실제 약물명 간의 정밀도, 재현율, F1 점수를 계산하여 데이터프레임의 행(row)에 추가합니다.
사용 예: calculate_metrics(row) → row에 precision, recall, f1_score 열 추가
